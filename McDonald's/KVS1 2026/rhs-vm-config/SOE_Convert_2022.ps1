##############################################################################
#
# SOE_Convert_2022.ps1 - converts a store's VM SOE in ONE run on RHS02.
#
# Run from the Provisioning Tool (or: powershell -ExecutionPolicy Bypass -File ...)
# AFTER AUSetup_RHS_SOE_VM_Config_2022.ps1 has created the "Server 2022 SOE" VM.
# Everything is derived from the RHS02 itself (site from the hostname, SOE IP from
# this box's 10.56.x.93 address); the only prompt is the SOE Administrator password.
# It sleeps through the VM's restarts and pauses only for the manual bits:
#   1. the restore screenshot   2. the guest setup wizard   3. the printer driver click-through
# SOE work is done over PowerShell Direct (Invoke-Command -VMName, VMBus) - no WinRM/c$
# needed; network remoting / c$ are fallbacks.
#
# v1.00 19/08/2026  Nick Barnes (Certeq) - initial
#
##############################################################################
param(
    [int]$Site = 0,                      # default: digits in the hostname (NZ00443RHS02 -> 443)
    [string]$SoeIp = '',                 # default: this box's 10.56.x.93 -> 10.56.x.1
    [string]$VMName = 'Server 2022 SOE', # as created by AUSetup_RHS_SOE_VM_Config_2022.ps1
...
$ptLogDir = 'C:\Program Files (x86)\McDonalds\McDonalds Provisioning Tool\Logs'
$logfile  = Join-Path $ptLogDir 'SOE_Convert.log'
$lines    = @()
$t0       = Get-Date

function Write-Log($Message) {
    $stamp = (Get-Date).ToString('dd/MM/yyyy HH:mm:ss')
    try { if (Test-Path $ptLogDir) { Add-Content -Path $logfile -Value "$stamp $Message" } } catch { }
    Write-Host $Message
}
function Add-Line($text) {
    $script:lines += $text
    $stamp = (Get-Date).ToString('dd/MM/yyyy HH:mm:ss')
    try { if (Test-Path $ptLogDir) { Add-Content -Path $logfile -Value "$stamp $text" } } catch { }
...
try { Start-Transcript -Path (Join-Path $env:TEMP 'SOE_Convert_transcript.txt') -Append | Out-Null } catch { }
...
if (-not $vm) {
    Add-Line "[FAIL] VM:            '$VMName' does not exist - run AUSetup_RHS_SOE_VM_Config_2022.ps1 from the Provisioning Tool first"
    Finish-Beer 'stopped'; Read-Host 'Press Enter to close' | Out-Null; exit 1
}
